...
class Modelagem{
	public int BuscaSequencial(Veiculo v[], int ident){
	int pos = -1;
	int tam = v.length;
	...
	String[] d = s.split("-");
	ano = Integer.parseInt(x[0]);
	mes = Integer.parseInt(x[1]);
	dia = Integer.parseInt(x[2]);
	}
	...
	//metodos
	public Veiculo(int i, int ma, int mo, int a, String cat, String[] comb, int co, double ca, String t, String tr, double cC, double cE, double c2, boolean t, Data dt){
	id = i;
	marca = m;
	modelo = mo;
	ano = a;
	...
	return String.format("[%d ## %s ## %s ## %d ## %s ## [%s] ## %d ## %.1f ## %s ## %s ## %.2f ## %.2f ## %.1f ## %s]",id, marca, modelo, ano, categoria, String.join(";",combustivel), cilindros, cilindrada, transmissao, tracao, consumoCidade, consumoEstrada, co2,turbo,dataRegistro.format());
	}
	  }
