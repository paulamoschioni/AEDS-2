...
class ListaDupla{
	...
	public void II(Veiculo v){
		Celula nova =  new Celula(v);
		//ajusta ponteiros da nova celula
		nova.ant = primeira;
		nova.prox = primeira.prox;
		
		//ajustando ponteiros da antiga primeira.prox
		primeira.prox.ant = nova;

		//ajustando ponteiros da celula cabeca
		primeira.prox = nova;
		
	}

   	public void IF(Veiculo v, int id){
		Celula nova = new Celula(v);  
		//ajusta ponteiros da nova celula
		nova.ant = ultima;
	...
	public void I(Veiculo v, int id, int pos){
		Celula nova = new Celula(v); 

		int j = -1;
		Celula atual = primeira;
		while(j < pos){	//para na celula anterior a que deve ser inserida
			atual = atual.prox;
			j++;
		}
		
		//ajustando ponteiros da celula nova
		nova.prox = atual.prox;
		nova.ant = atual;

		//ajustando ponteiro da atual.prox
		atual.prox.ant = nova;

		//ajustando ponteiro da atual
		atual.prox = nova;
	}
	public Veiculo RI(){
		Celula alvo = primeira.prox;
		Veiculo temp = alvo.veiculo;
		
		//ajustar ponteiros de primeira
		primeira.prox = alvo.prox;

		//ajustar ponteiros de primeira.prox
		if (primeira.prox == null) {
       		 //lista ficou vazia
       		 ultima = primeira;
    		} else {
      		  primeira.prox.ant = primeira; // novo primeiro aponta pra trás, pra primeira
 		   }
	...
	public Veiculo R(int pos){
		int j = -1;
		Celula atual = primeira;
	
		while(j < pos){ //para na celula 
			atual = atual.prox;
			j++;
		}

		Veiculo temp = atual.veiculo;	//guarda veiculo
...
public static void main(String[] args) throws FileNotFoundException {
	...
	int id;
        ...
        for (int p = 0; p < operacoes; p++) {
            ...
            if (comando.equals("R* ")) {
		lerPosicao = sc.nextInt();
                removidos[r] = lista.R(lerPosicao);
                r++;
